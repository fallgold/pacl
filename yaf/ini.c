...
/* $Id: ini.c 324896 2012-04-06 09:46:37Z laruence $ */
...
static void value2element(zval* tmparr, zval* value, zval** element) {
	if (IS_STRING == Z_TYPE_P(value)) {
		char *svalue = estrndup(Z_STRVAL_P(value), Z_STRLEN_P(value));
		char *item = svalue, *p = svalue, *end = svalue + Z_STRLEN_P(value);
		while (p < end) {
			p = strchr(item, '|');
			if (NULL != p) {
				while (*--p == ' ' || *p == '\t') // rtrim
			...
			while (*item == ' ' || *item == '\t') // ltrim
				item++;
			if (strlen(item)) {
				MAKE_STD_ZVAL(*element);
				ZVAL_STRING(*element, item, 1);
				add_next_index_zval(tmparr, *element);
			}
			if (NULL == p)
				break;
			item = ++p;
					...
					do {
						...
						if (zend_symtable_find(Z_ARRVAL_P(dst), real_key, strlen(real_key) + 1, (void **) &ppzval) == FAILURE) {
							if (seg) {
								...
								value2element(tmparr, value, &element);
								zend_symtable_update(Z_ARRVAL_P(dst), 
										real_key, strlen(real_key) + 1, (void **)&tmparr, sizeof(zval *), NULL);
								break;
							}
						} else {
							if (IS_ARRAY != Z_TYPE_PP(ppzval)) {
								...
								if (seg) {
									...
									/*MAKE_STD_ZVAL(element);*/
									/*ZVAL_ZVAL(element, value, 1, 0);*/
									value2element(*ppzval, value, &element);
									/*add_next_index_zval(*ppzval, element);*/
								}
...
*/
STU_MINIT_FUNCTION(config_ini) 
{
	zend_class_entry ce;

	INIT_CLASS_ENTRY(ce, "PACL_Yaf_Config_Ini", yaf_config_ini_methods);
